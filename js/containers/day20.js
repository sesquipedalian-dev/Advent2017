...
class Particle { 
    constructor(id, inputLine) { 
        this.id = id;

        // sample input:
        // p=<-317,1413,1507>, v=<19,-102,-108>, a=<1,-3,-3>
        const inputRegex = /p=<\s*([\-\d]+),\s*([\-\d]+),\s*([\-\d]+)>, v=<\s*([\-\d]+),\s*([\-\d]+),\s*([\-\d]+)>, a=<\s*([\-\d]+),\s*([\-\d]+),\s*([\-\d]+)>/;
        const match = inputLine.match(inputRegex);
        if(match) { 
            this.position = new Vector3(parseInt(match[1]), parseInt(match[2]), parseInt(match[3]));
            this.velocity = new Vector3(parseInt(match[4]), parseInt(match[5]), parseInt(match[6]));
            this.acceleration = new Vector3(parseInt(match[7]), parseInt(match[8]), parseInt(match[9]));
        } else { 
    ...
    getManhattanAcceleration() { 
        return Math.abs(this.acceleration.x) + 
...
class Day20 extends Component {
    ...
    solveInput1(input) { 
        ...
        if(typeof input1 === 'string') {
            ...
            const particles = lines.map((l, i) => new Particle(i, l));
            particles.sort((a, b) => {
                return a.getManhattanAcceleration() - b.getManhattanAcceleration();
            });
            
            const targetAccel = particles[0].getManhattanAcceleration();
            const equalAccelerationParticles = particles.filter(p => p.getManhattanAcceleration() == targetAccel);

            equalAccelerationParticles.sort((a, b) => { 
                const veloDiff = a.getManhattanVelocity() - b.getManhattanVelocity();
                if(veloDiff == 0) { 
            ...
            this.props.setDay20Output1(equalAccelerationParticles[0].id);
        }
    }
    ...
    render() {
        ...
        return (
            <ScrollView>
                <Text>
                    manhattan acceleration
                </Text>
                <TextInput
                    multiline={true}
                    value={this.props.input1}
                    onChangeText={(text) => this.props.setDay20Input1(text)}
                    returnKeyType='none'
                />
                <Button
...
export default comp = defineView(Day20, (state) => {
    return { 
        input1: state.day20Input1, 
        output1: state.day20Output1,
        output2: state.day20Output2,
    }
});
